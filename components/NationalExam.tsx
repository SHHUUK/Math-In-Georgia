...
import { 
  ...
  CheckCircle2, FileText, Loader2, Trophy, XCircle, Play, Calculator
} from 'lucide-react';
...
export const NationalExam: React.FC = () => {
  ...
  const handleSubmitExam = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // Sort details by original question order
      details.sort((a, b) => {
      ...
      alert("დაფიქსირდა შეცდომა შედეგების დათვლისას.");
      setGameState('active');
    }
  };
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center p-6 animate-fadeIn text-center">
        <div className="bg-white p-10 rounded-3xl shadow-xl border border-slate-200 max-w-2xl w-full">
          <div className="w-24 h-24 bg-indigo-50 rounded-full flex items-center justify-center mx-auto mb-6 text-indigo-600"><FileText size={48} /></div>
          ...
          <button type="button" onClick={startExam} className="w-full py-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold text-lg shadow-lg transition-all active:scale-[0.98] flex items-center justify-center gap-2">
            <Play size={24} fill="currentColor" /> გამოცდის დაწყება
          </button>
        </div>
      </div>
    );
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center animate-fadeIn gap-6">
         <div className="relative">
            ...
            <div className="absolute inset-0 flex items-center justify-center"><Loader2 className="w-10 h-10 text-indigo-600 animate-spin" /></div>
         </div>
    ...
    return (
      <div className="h-full flex flex-col items-center justify-center animate-fadeIn gap-6">
         <div className="relative">
            ...
            <div className="absolute inset-0 flex items-center justify-center"><Loader2 className="w-10 h-10 text-green-600 animate-spin" /></div>
         </div>
         <div className="text-center">
            <h2 className="text-xl font-bold text-slate-800">ნაშრომი სწორდება...</h2>
            <p className="text-slate-500 mt-2">AI ამოწმებს ღია დავალებებს</p>
         </div>
      </div>
    );
  }
       ...
       <div className="max-w-5xl mx-auto p-4 md:p-8 animate-fadeIn pb-20 font-sans">
         <div className="bg-white p-8 rounded-3xl shadow-xl border border-slate-200 mb-8 text-center relative overflow-hidden">
            ...
            <div className="flex justify-between items-start mb-6 relative z-10">
               <div className="bg-slate-100 px-4 py-2 rounded-xl flex items-center gap-2 font-mono text-slate-900 font-bold"><Clock size={20} />{formatTime(timeTaken)}</div>
            ...
            <div className="mb-2">
               ...
               <p className="text-slate-500 mt-1">მინიმალური გამსვლელი ქულა: <span className="font-bold text-slate-900">11</span></p>
               <p className="text-slate-500 mt-1">მაქსიმალური ქულა: <span className="font-bold text-slate-900">{result.maxScore}</span></p>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8 mt-8">
               <div className="p-5 bg-indigo-50 rounded-2xl border border-indigo-100"><div className="text-4xl font-bold text-indigo-600">{result.totalScore}</div><div className="text-xs text-indigo-400 font-bold uppercase mt-1">ჯამური ქულა</div></div>
               <div className="p-5 bg-green-50 rounded-2xl border border-green-100"><div className="text-4xl font-bold text-green-600">{correctCount}</div><div className="text-xs text-green-500 font-bold uppercase mt-1">სწორი</div></div>
               <div className="p-5 bg-yellow-50 rounded-2xl border border-yellow-100"><div className="text-4xl font-bold text-yellow-600">{partialCount}</div><div className="text-xs text-yellow-600 font-bold uppercase mt-1">ნაწილობრივ</div></div>
               <div className="p-5 bg-red-50 rounded-2xl border border-red-100"><div className="text-4xl font-bold text-red-600">{wrongCount}</div><div className="text-xs text-red-400 font-bold uppercase mt-1">არასწორი</div></div>
            </div>
         </div>
         <div className="space-y-4">
            <h3 className="font-bold text-xl text-slate-800 ml-2 flex items-center gap-2 mb-6"><FileText size={24} className="text-indigo-600" /> დეტალური გარჩევა</h3>
            {questions.map((q, idx) => {
                 ...
                 <div key={q.id} className={`p-6 rounded-2xl border transition-all ${isFullScore ? 'bg-white border-green-200 shadow-sm' : isZero ? 'bg-white border-red-200 shadow-sm' : 'bg-white border-yellow-200 shadow-sm'}`}>
                    ...
                    <div className="bg-slate-50 p-5 rounded-xl text-sm border border-slate-100">
                       <div className="mb-4">
                          ...
                          <div className={`p-3 rounded-lg bg-white border border-slate-200 text-slate-900 ${q.type === 'mc' ? 'font-bold' : 'font-mono whitespace-pre-wrap'}`}>
                             {q.type === 'mc' ? (q.options ? <MathRenderer text={q.options[parseInt(det.userAnswer)] || "პასუხი არ არის"} inline /> : '-') : <MathRenderer text={det.userAnswer || "პასუხი არ არის"} />}
                          </div>
                       </div>
                       {!isFullScore && (
                          <div className="pt-3 border-t border-slate-200">
                             <strong className="text-slate-500 uppercase text-xs tracking-wider block mb-2">სწორი პასუხი / კომენტარი:</strong>
                             <div className="text-slate-900 bg-green-50/50 p-3 rounded-lg border border-green-100"><MathRenderer text={det.feedback} /></div>
                          </div>
                       )}
                    </div>
                 </div>
         ...
         <div className="mt-12 text-center pb-10">
            <button type="button" onClick={() => setGameState('intro')} className="px-10 py-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold shadow-lg transform hover:scale-105 transition-all">მთავარზე დაბრუნება</button>
         </div>
       </div>
     );
  }
  ...
  return (
    <div className="h-full flex flex-col bg-slate-50 font-sans text-slate-900">
       <div className="bg-white border-b border-slate-200 px-6 py-4 flex justify-between items-center shadow-sm z-20 sticky top-0">
          <div className="flex items-center gap-3 text-slate-900 font-mono font-bold text-xl bg-slate-50 px-4 py-2 rounded-lg border border-slate-100">
             <Clock size={24} className={timeLeft < 600 ? 'text-red-500 animate-pulse' : 'text-indigo-600'} />
             {formatTime(timeLeft)}
          </div>
          <div className="hidden md:flex flex-col items-end">
             <span className="text-xs font-bold text-slate-400 uppercase tracking-widest">NAEC მათემატიკის სიმულაცია</span>
             <span className="text-xs font-bold text-indigo-600 uppercase tracking-widest mt-1">სულ ქულა: {calculateMaxPoints()} | ბარიერი: 11</span>
          </div>
       </div>
          ...
          <div className="w-20 md:w-72 bg-white border-r border-slate-200 overflow-y-auto p-4 hidden md:block custom-scrollbar">
             ...
             <div className="grid grid-cols-5 gap-2">
                {questions.map((q, idx) => {
                   ...
                   return (
                     <button 
                       key={q.id} type="button" onClick={() => navigate(idx)}
                       className={`aspect-square rounded-lg flex items-center justify-center text-sm font-bold transition-all ${isActive ? 'bg-indigo-600 text-white ring-2 ring-indigo-300 scale-110 z-10 shadow-md' : isAnswered ? 'bg-indigo-50 text-indigo-700 border border-indigo-100' : 'bg-slate-50 text-slate-400 hover:bg-slate-100 border border-slate-100'}`}
                     >{idx + 1}</button>
                   );
                })}
             </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 md:p-12 w-full bg-slate-50/50">
             <div className="max-w-3xl mx-auto bg-white p-8 md:p-10 rounded-3xl shadow-sm border border-slate-200 min-h-[500px] flex flex-col">
                <div className="mb-8 flex justify-between items-center border-b border-slate-100 pb-4">
                   ...
                   <span className="bg-indigo-50 text-indigo-600 px-4 py-1.5 rounded-full text-xs font-bold border border-indigo-100 shadow-sm">{currentQ.points} ქულა</span>
                </div>
                <h2 className="text-2xl md:text-3xl font-bold text-slate-900 mb-10 leading-snug"><MathRenderer text={currentQ.text} /></h2>
                <div className="flex-1">
                   {currentQ.type === 'mc' && currentQ.options ? (
                      <div className="space-y-4">
                         {currentQ.options.map((opt, idx) => (
                            <button
                              key={idx} type="button" onClick={() => handleAnswerChange(idx.toString())}
                              className={`w-full p-5 rounded-2xl border-2 text-left flex items-center gap-5 transition-all group ${userAnswers[currentQ.id] === idx.toString() ? 'border-indigo-600 bg-indigo-50 shadow-md' : 'border-slate-100 bg-white hover:border-indigo-200 hover:bg-slate-50'}`}
                            >
                               <div className={`w-8 h-8 rounded-full border-2 flex items-center justify-center shrink-0 transition-colors ${userAnswers[currentQ.id] === idx.toString() ? 'border-indigo-600 bg-indigo-600' : 'border-slate-300 group-hover:border-indigo-300'}`}>
                                  {userAnswers[currentQ.id] === idx.toString() && <div className="w-3 h-3 bg-white rounded-full" />}
                               </div>
                               <span className={`text-lg font-medium ${userAnswers[currentQ.id] === idx.toString() ? 'text-indigo-900' : 'text-slate-900'}`}><MathRenderer text={opt} inline /></span>
                            </button>
                         ))}
                      </div>
                   ) : (
                      <div className="space-y-4 h-full flex flex-col">
                         <div className="bg-amber-50 border border-amber-100 p-4 rounded-xl flex gap-3 items-start text-amber-900 text-sm mb-2">
                            <AlertCircle size={20} className="shrink-0 mt-0.5 text-amber-600" />
                            <p>ეს არის ღია დავალება. ჩაწერეთ თქვენი პასუხი და მსჯელობა ვრცლად.</p>
                         </div>
                         <textarea
                           value={userAnswers[currentQ.id] || ''}
                           onChange={(e) => handleAnswerChange(e.target.value)}
                           placeholder="ჩაწერეთ პასუხი..."
                           className="flex-1 w-full min-h-[200px] p-6 bg-white text-slate-900 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-indigo-500 outline-none text-lg resize-y transition-shadow focus:bg-white"
                         />
                      </div>
                   )}
                </div>
                <div className="mt-12 flex justify-between items-center pt-8 border-t border-slate-100">
                   <button type="button" onClick={() => navigate(currentQIndex - 1)} disabled={currentQIndex === 0} className="flex items-center gap-2 px-6 py-3 rounded-xl font-bold text-slate-500 hover:bg-slate-100 hover:text-slate-800 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"><ChevronLeft size={22} /> წინა</button>
                   {currentQIndex === questions.length - 1 ? (
                      <button type="button" onClick={(e) => handleSubmitExam(e)} className="flex items-center gap-2 px-8 py-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold shadow-lg transition-all transform hover:scale-105">დასრულება და შეფასება <CheckCircle2 size={22} /></button>
                   ) : (
                      <button type="button" onClick={() => navigate(currentQIndex + 1)} className="flex items-center gap-2 px-8 py-4 bg-slate-900 hover:bg-slate-800 text-white rounded-xl font-bold shadow-lg transition-all transform hover:scale-105">შემდეგი <ChevronRight size={22} /></button>
                   )}
                </div>
             </div>
          </div>
